...
SELECT DISTINCT cust_name
FROM restBill
WHERE cust_name LIKE '% C%'; -- Assumes the format "First Last"

SELECT CONCAT(first_name, ' ', surname) AS headwaiter_name
FROM restStaff
WHERE headwaiter IS NULL; -- Assuming headwaiter has no parent reference

SELECT *
FROM restBill
WHERE bill_date BETWEEN 160201 AND 160229; -- Assuming YYYYMMDD format

SELECT DISTINCT bill_date
FROM restBill
WHERE bill_date BETWEEN 150101 AND 151231 -- Assuming YYYYMMDD format
ORDER BY bill_date;
